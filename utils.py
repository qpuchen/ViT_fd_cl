...
def parse_option():
    ...
    parser.add_argument('--data_path', default='train data path', type=str, metavar='PATH',
                        help='root of train data folder, the full path is <train_data>> (default: train_data)')
    ...
    parser.add_argument('--drop_w', type=float, default=0.5,
...
def save_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, fold_index=0):
    ...
    # save_path = os.path.join(config.OUTPUT, f'ckpt_epoch_{epoch}.pt')
    model_path = os.path.join(config.OUTPUT, "tmp_data")
    os.makedirs(model_path, exist_ok=True)
    save_path = os.path.join(model_path, f'best_{fold_index}.pt')
