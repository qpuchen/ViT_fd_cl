...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        num_features = [256, 512, 1024, 1024]
        self.norms = nn.ModuleList()
        for i_layer in range(self.num_layers):
            norm = norm_layer(num_features[i_layer])
            self.norms.append(norm)
        self.head_1 = nn.Linear(256 + 512 + 1024 * 2, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def fpn_predict_down(self, x: dict, logits: dict):
        ...
        for name in x:
            ...
            logits[name] = logits[name].transpose(1, 2).contiguous().float()  # transpose
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
            x_ = self.norms[i - 1](x)
            x_ = self.avgpool(x_.transpose(1, 2))
        ...
        x = torch.flatten(x, 1)
        return x, x_combined, feat_dict

    def forward(self, x):
        x_, x_combined, feat_dict = self.forward_features(x)
        x = self.head(x_)
        x_c = self.head_1(x_combined)

        logits_dict = {}
        if self.use_fpn:
            proj_type = "Linear"
            upsample_type = "Conv"
            self.fpn_down = FPN(feat_dict, self.fpn_size, proj_type, upsample_type)
            self.build_fpn_classifier_down(feat_dict, self.fpn_size, self.num_classes)
            self.fpn_up = FPN_UP(feat_dict, self.fpn_size)
            self.build_fpn_classifier_up(feat_dict, self.fpn_size, self.num_classes)

            feat_dict = self.fpn_down(feat_dict)
            # print([name for name in x])
            self.fpn_predict_down(feat_dict, logits_dict)
            feat_dict = self.fpn_up(feat_dict)
            self.fpn_predict_up(feat_dict, logits_dict)

        if self.use_selection:
            w_fpn_size = self.fpn_size if self.use_fpn else None  # if not using fpn, build classifier in weakly selector
            self.selector = WeaklySelector(feat_dict, self.num_classes, self.num_selects, w_fpn_size)
            logits_dict = self.selector(feat_dict, logits_dict)

        return x, x_, x_c, x_combined, logits_dict
        # return x
    ...
    def forward(self, x, logits=None):
        ...
        for name in x:
            ...
            for bi in range(logits[name].size(0)):
                ...
                preds_1.append(logits[name][bi][ranks[:num_select]].float())
            ...
            logits["select_" + name] = preds_1.float()
    ...
    def __init__(self, inputs: dict, fpn_size: int, proj_type: str, upsample_type: str):
        ...
        for i, node_name in enumerate(inputs):
            ...
            if upsample_type == "Conv" and i != 0:
                ...
                # if in_dim != out_dim:
                m = nn.Conv1d(in_dim, out_dim, 1).cuda()  # for spatial domain
                # else:
                #     m = nn.Identity()
