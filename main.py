...
def main(config):
    ...
    for fold_index in range(5):
        ...
        data_loader_train, config.MODEL.NUM_CLASSES = build_loader(config, logger=logger, is_pretrain=False,
                                                                   is_train=True, fold_index=fold_index)
        config.freeze()
        data_loader_val, _ = build_loader(config, logger=logger, is_pretrain=False, is_train=False, fold_index=fold_index)
        mixup_fn = None
        ...
        model_path = os.path.join(config.OUTPUT, "model_data")
        os.makedirs(log_path, exist_ok=True)
        if config.TRAIN.AUTO_RESUME:
            resume_file = auto_resume_helper(model_path)
            if resume_file:
                ...
                logger.info(f'no checkpoint found in {model_path}, ignoring auto resume')
                ...
                if max_accuracy < acc1:
                    ...
                    if dist.get_rank() == 0:
                        save_checkpoint(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler,
                                        loss_scaler, logger, fold_index=fold_index)
                # max_accuracy = max(max_accuracy, acc1)
            logger.info(f'Max accuracy: {max_accuracy:.2f}% in epoch:{best_epoch}.')

        model_path = os.path.join(config.OUTPUT, "model_data")
        save_path = os.path.join(config.OUTPUT, "tmp_data", f'best_{fold_index}.pt')
        shutil.move(save_path, model_path)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler):
    ...
    for idx, batch in enumerate(data_loader):
        ...
        loss_con = torch.tensor(0.0).cuda(non_blocking=True)
        if config.TRAIN.CON:
...
if __name__ == '__main__':
    ...
    log_path = os.path.join(config.OUTPUT, "tmp_data")
    os.makedirs(log_path, exist_ok=True)
    logger = create_logger(output_dir=log_path, dist_rank=dist.get_rank(), name=f"{config.MODEL.NAME}")
