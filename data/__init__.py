...
def build_loader(config, simmim=False, is_pretrain=False, logger=None, is_train=False, fold_index=0):
    if not simmim:
        return _build_loader(config, logger=logger, is_train=is_train, fold_index=fold_index)
    if is_pretrain:
