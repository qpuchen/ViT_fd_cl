...
class Cultivar():
    def __init__(self, root, is_train=True, transform=None, fold_index=0):
        self.root = root
        ...
        # Read data from AAA.txt
        with open(os.path.join(self.root, 'anno/label.txt'), 'r') as f:
            for line in f:
        ...
        folds = list(skf.split(self.imgs_name, self.labels))
        train_indices, test_indices = folds[self.fold - 1]

        if self.fold < 0 or self.fold > 4:
            raise ValueError("Fold must be between 1 and 5")

        self.train_imgs = [self.imgs_name[i] for i in train_indices]
        self.test_imgs = [self.imgs_name[i] for i in test_indices]
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            img_path = os.path.join(self.root, 'images', self.train_imgs[index])
            target = self.labels[self.imgs_name.index(self.train_imgs[index])]
        else:
            img_path = os.path.join(self.root, 'images', self.test_imgs[index])
            target = self.labels[self.imgs_name.index(self.test_imgs[index])]
